...
import { useState } from 'react';
 ...
 const countriesData = [
   ...
   const CountriesComponent = () => {
    ...
    const handleClick = () => {
      setCount(Math.round(Math.random() * countriesData.length));
    }

    const country = countriesData.map((countr) => {
        const name = countr.name;
        const capital = countr.capital;
        const languages = countr.languages;
        const population = countr.population;
        const flag = countr.flag;
        const currency = countr.currency;
        ...
        return (
            <div className='random-country' key={name}>
                <img src={flag} alt='flag' />
                <h2 key={name}>Country: {name}</h2>
                <p><b> Capital</b>: {capital}</p>
                <p><b>Languages</b>: {languages.join(' ')}</p>
                <p><b>Population</b>: {formattedPopulation}k</p>
